...
const MessagesList = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10">
      <h1 className="text-2xl font-bold text-center text-gray-800 mb-4">Received Messages</h1>
      <ul className="divide-y divide-gray-300">
        {messages.map((message) => (
          <li key={message.id} className="p-4 hover:bg-gray-50 transition duration-150 ease-in-out">
            <p className="text-gray-600"><strong>From:</strong> {message.sender_number}</p>
            <p className="text-gray-900"><strong>Message:</strong> {message.text}</p>
          </li>
        ))}
      </ul>
    </div>
